...
| **Pending contract change** | When that lands, a proxied run passes `--secret atelier-claude-token,type=env,target=CLAUDE_CODE_OAUTH_TOKEN` (final name TBC) and **stops mounting `.credentials.json` in the claude volume**. `lib/container.sh`'s claude branch changes from a volume mount to a secret; the codex and gh volumes are unchanged. Nothing in Meute should be built against the current claude mount without knowing it is going away | Atelier, awaiting owner's go |
